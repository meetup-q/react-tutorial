...
造成使用者體驗中斷。
而 SPA 則是只有一個 HTML 頁面，所以只需要 initial load 一次，
所有的內容更新都由 JavaScript 控制，
不再需要重新載入整個頁面。
SPA 還有一個優點就是最小化 request 次數，
因為每次只更新需要更新的部分，
...
造成 SPA 網站被認為是空的。
入口網站公司則以延長快照時間來對應這個問題，
我們 developer 也可以透在 HTML 檔中寫好網站重點資訊來避免。
二是 SPA 會導致「上一頁」失效，
這是一個非常致命的缺陷，因為使用者已經非常習慣使用上一頁來回到頁面上一個狀態，
但 SPA 實際上只有一頁，每次互動對頁面產生的變化都是 JS 造成的，
上一動的狀態並不會被紀錄下來。
這一點可以透過 router 模組來解決。
...
one way data flow 是 react 的運作機制，
components 的資料只能往 children nodes 傳下去，不能逆流而上。
這個機制是為了確保資料更新不混亂。
這個機制的缺點是資料交換很麻煩，所以出現了 state 儲存中心的工具例如 redux。

### Component Lifecycle

component lifecycle 是 component 的生命週期，
developer 可以依照需求在 component 的某個生命週期執行對應的指令，
lifecycle 的主要分類如下：

2. componentDidMount()
3. componentWillMount()

lifecycle 並非 react 創的特色，
html5 的 createElement 也有類似的功能，
我們在學習 framework 之餘也應該學習 plain html & javascript。

### Declaretive

React 是 declarative 的工具，意思是我們只要告訴它結果，中間的過程它會自己想辦法。
這並不算是一個功能，但是一個很值得注意的特點。
在後面三堂課我們實際操作時，
就能深刻地感受到這個特點帶來的好處。


如下面這個例子，這是一個紀錄與顯示 button 被點擊的次數的 component，
...
``` 
如果使用 jQuery 就得這樣寫，
可以發現為了把更新後的 clickTimes render 上去，
我們必須在 on click 的函數裡再寫一次 render 的程式，
為了完成這個功能，同一個 part 就重複出現兩次了。
這就是 imperative 工具的矮板，我得告訴它在什麼事情發生之後，
要接著做什麼事情。 
